...
**能不能在 Excel 里试算，取决于它是「乘一个数」还是「换一套算法」。**

| 类 | 例 | 怎么变 |
|---|---|---|
| 乘一个数 | 生产率、应用类型/开发类别系数、**项目特征因子** | `03_二层试算` 直接改，即时重算 |
| 加减一批行 | 交付范围 | `04_三层试算` 直接改 |
| 换一套分派 | 交付形态（CE-DEV vs CE-SUB vs 不计） | 看 Z04 `02_逐子系统场景对比` —— 预置场景，引擎逐个算 |
| **换第二层口径** | 计数方法、项目类型 | **必须重建基准**，工具会拒绝出报价 |

最后一类不是缺陷，是分层的兑现。实测 `--project-type 升级改造`：

```
与基准不一致：software_dev.afp_total 引擎 9441.53 vs 基准 12221.83
若确实换了计数方法/项目类型，须先重建基准。
```

`counting_method` 写进 `deal.yaml` 只作**声明与校验** —— 与基准不符会报错并
给出重建命令，**不会静默生效**。此前它会被基准覆盖，属「声明了不生效」，
比不支持更糟：人以为改了。
...
**Z00–Z04 之间没有跨册公式链接，这是刻意的。** 三条理由：

1. 送审文件名带 `v0.18.0_20260806` —— **每次重跑都变**。外链
   `='[…_Z02_…_v0.18.0_20260806.xlsx]04_三层试算'!G25` 下个版本必断
2. 送审是**分册递交**，评审可能只拿到 Z00 一册；带外链的册单独打开是 `#REF!`
3. 更要紧：让试算传播到 Z00，就制造了一份**看起来像送审值但不是**的总表 ——
   那正是这套工具处处在防的

替代做法：`04_三层试算` 末尾镜像 Z00 的六大类，**全部活公式**，
标明「这是试算结果，不是送审值」。实测勾掉一个子系统：

    软件开发费   10,554,395.02 → 9,706,012.42   （−848,382.60）
    其他建设费用    357,600.00 →   341,300.00   （ −16,300.00）
    建设期合计   11,712,465.02 → 10,847,782.42  （−864,682.60）

与引擎 `scope` 排除后逐位一致。**范围定下来要改 `deal.yaml` 重跑** ——
重跑后五册一起更新，这比外链可靠。
