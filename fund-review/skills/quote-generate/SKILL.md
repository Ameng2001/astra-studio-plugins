...
| `…_Z02_功能点测算表_….xlsx` | `00_测算汇总` / `01_功能点明细` / `02_计价参数` / `03_试算`（汇总排在明细前，评审先看钱） |

### 试算：改参数看全盘影响

`02_计价参数` 是参数面板，`03_试算` 逐子系统用**活公式**引用它的 C 列 ——
...
形态切换是分派逻辑，Excel 表达不了（见 `04_交付试算` 的说明）。
...
`03_试算` 试的是**算法参数**（生产率、应用类型系数）；
`04_交付试算` 试的是**商机范围** —— 把某个子系统的「本次在范围内」
改成「否」，建设期合计即时重算。
...
| 子系统级 | 每个子系统 | `03_试算` 逐行白格 | 应用类型、开发类别，15 份各不相同 |
