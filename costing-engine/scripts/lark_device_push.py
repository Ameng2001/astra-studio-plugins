...
_HUE = {
    ...
    # Purple 而非 Violet：飞书单选色只认 Red/Orange/Yellow/Lime/Green/
    # Turquoise/Wathet/Blue/Carmine/Purple/Gray 这 11 个。Violet 会让
    # **整张表建不出来**，且错误被建表调用吞掉、表现为「轮询 8 次找不到」。
    "★字段冲突": ("Carmine", "Light"), "★待申请料号": ("Purple", "Light"),
    "★未登记": ("Red", "Light"),
...
def build_rows(root: Path) -> dict[str, list[dict]]:
    ...
    # v2 起价格与物料**分文件**：materials.yaml 推共创，pricing.yaml 不推。
    # 所以这里可能整批拿不到 price_status / ref_vendors —— 那不是数据缺失，
    # 是设计。判定一次、全表统一，避免逐条 .get() 掩盖掉「本该有价却没有」
    # 的 v1 数据损坏（v1 层每条都有 price_status，缺了就是真出事了）。
    priced_layer = any("price_status" in m for m in mats)
    if not priced_layer:
        print("  · 无价模式：物料表不含价格字段（v2 价格已拆到 pricing.yaml，"
              "不推共创表）—— 「★待核价」判据与三列价格列本次不参与")
    ...
    for m in mats:
        ...
        elif priced_layer and m["price_status"].startswith("**"):
...
def main() -> None:
    ...
    for t, tid in made.items():
        ...
        for v in ((r.get("data") or {}).get("items")
            ...
            # **按表里实际存在的列过滤**。VIS 是照 v1 的飞书表写的：那边
            # 主字段被人工改名成「标准设备名称」、还有两列价格。v2 新建的
            # 表叫「设备名称」且价格已拆出，payload 里带上不存在的列名会让
            # 飞书**整批拒绝**，表现为「设了但一列没变」（got 仍是全字段）。
            exist = {f if isinstance(f, str) else f["name"]
                     for f in lt.fields(bt, tid)}
            # 主字段在 v1 飞书上被人工改名过（设备名称 → 标准设备名称）。
            # 名字是展示字段不是键 —— 两边都接住，别让一次改名把主字段
            # 从共创面上滤掉。
            alias = {"标准设备名称": "设备名称", "设备名称": "标准设备名称"}
            cols = [c if c in exist else alias.get(c, c) for c in cols]
            cols = [c for c in cols if c in exist]
