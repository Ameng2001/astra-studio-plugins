...
def ensure_table(bt: str, name: str, fields: list[dict]) -> str:
    ...
    # **建表的返回要看**。它失败时（字段规格非法、名字冲突、超上限）如果
    # 只是往下走，8 秒后报出来的是「轮询找不到」—— 一句把「没建成」说成
    # 「建慢了」的假陈述，真错误（实测：单选色 'Violet' 不在飞书枚举里）
    # 被埋在没人看的返回体里。宁可当场炸在真正的原因上。
    rc = lark("base", "+table-create", "--base-token", bt, "--name", name,
    ...
    if isinstance(rc, dict) and rc.get("ok") is False:
        ...
        raise LarkTableError(
            f"{name}：建表被飞书拒绝 —— {err.get('message') or rc}"
...
def main() -> None:
    ...
    bt = cfg["base_token"]
    # **表名优先取 config 的 sheet_names**（v2 分层：各层自管表名，
    # 同一子系统名在不同层可以有不同表号），回落到 SHEET_ORDER（v1 单库）。
    # 不这样做的话，v2 每加一层就要回来改这份硬编码表，而漏改的表现是
    # 「未登记的子系统」硬失败 —— 至少是当场炸，但每次都要改代码。
    _sn = cfg.get("sheet_names") or {}
    if _sn:
        SHEET_ORDER.update(_sn)
    bom = Bom.load(a.bom)
    ...
    for _s, _it in by_sys.items():
        ...
        if not _n or not _want(_n) or _n not in tables:
            continue
        # 表还没建（新库首推）时跳过预检 —— 预检的前提是「已有表的字段
        # 与当前规格一致」，表不存在时没有可比对的对象。ensure_table 会按
        # 规格建表建字段，建出来的必然一致。
        if not tables.get(_n):
            continue
        try:
