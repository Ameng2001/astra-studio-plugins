...
def _stage_rates(pack) -> dict[str, float]:
    """各阶段人月费率（万元/人月）。

    **不是所有标准都分阶段**：柳州标准表1 给需求分析/系统设计/编码/测试/实施
    五档（1.9/1.9/1.7/1.6/1.4 万），广东标准只给统一人月单价（2.4 万）。
    缺分阶段费率时按统一单价、各阶段同价 —— 这不是兜底猜值，是标准本身
    就没有分档：按统一价计与按标准算出的总额一致，只是阶段列不再有价差。

    两者都没有则硬失败：人月折不成钱，静默按 0 计会让工作量法那段整段消失。
    """
    em = pack.data.get("effort_method") or {}
    sr = em.get("stage_rates") or {}
    if sr:
        return {k: v["value"] if isinstance(v, dict) else v for k, v in sr.items()}
    base = ((pack.data.get("rates") or {}).get("base_man_month_rate") or {})
    val = base.get("value") if isinstance(base, dict) else base
    if not val:
        raise gs.GovSheetError(
            f"{pack.pack_id} 既无 effort_method.stage_rates 也无 "
            f"rates.base_man_month_rate —— 人月折不成金额，工作量法那段无法计价")
    wan = float(val) / 10000.0
    return {st: wan for st in STAGES}
...
def _make_bucket(tax: dict):
    ...
    # 分节规则**由 taxonomy 声明**，不硬编码产品线名。
    # 曾经写死幼教的三个产品线名（"数智幼教行业大模型"/"数智民生生态体系平台
    # 支撑基座"，其余落兜底），换个行业或改个产品线名，全部子系统就塌进
    # 兜底那一节 —— 总表变成「一个平台 2,280 万」而不是分层明细。
    # 金额总数不变，所以不会有任何校验报错，只有人肉看表才发现。
    #
    # taxonomy 里每个产品线可声明：
    #   summary_order  该产品线在总表的排序（缺省按出现顺序）
    #   summary_split  true = 该产品线下每个子系统各占一行（工作量法各段用）
    order_of = {}
    split_lines = set()
    meta_seq = {}
    for _l, _ls in (tax.get("product_lines") or {}).items():
        for _s, _sp in (_ls.get("systems") or {}).items():
            _sp = _sp if isinstance(_sp, dict) else {}
            if _sp.get("summary_seq") is not None:
                meta_seq[_s] = _sp["summary_seq"]
    for _i, (line, ls) in enumerate((tax.get("product_lines") or {}).items(), 1):
        order_of[line] = ls.get("summary_order", _i)
        if ls.get("summary_split"):
            split_lines.add(line)

    def bucket(sysname):
        line, layer = meta.get(sysname, ("其他", ""))
        base = order_of.get(line, 99)
        if line in split_lines:
            # 逐子系统一行。**行序由 taxonomy 的 summary_seq 声明**，
            # 缺省才按名称排 —— 按名称排会让「知识工程 / 本体工程 / 数据工程」
            # 变成「数据 / 本体 / 知识」，金额一分不差但评审看到的顺序变了，
            # 而总表行序是要跟历史版本对得上的。
            seq = (meta_seq.get(sysname)
                   or 50 + sorted(n for n, (l, _) in meta.items()
                                  if l == line).index(sysname))
            return (base + seq / 100.0,
                    f"{line}-{sysname}（{layer}）" if layer else f"{line}-{sysname}")
        return (base, f"{line}（{layer}）" if layer and layer != "应用层" else line)
...
def emit_software(wb, sw: dict, deal: dict, pack: StandardPack,  # noqa: C901
    ...
    """
    # 条款出处：分阶段费率的标准（柳州）记在 effort_method，
    # 只给统一人月单价的标准（广东）记在 rates.base_man_month_rate。
    # 取不到就留空 —— 出处宁可缺，不可指错条款。
    cite = ((pack.data.get("effort_method") or {}).get("citation")
            or ((pack.data.get("rates") or {}).get("base_man_month_rate") or {}).get("citation")
            or {})
    ...
    path.write_text(json.dumps({
        ...
        "deal_decisions": {
            ...
            "hardware_passthrough": bool(
                (deal.get("sources") or {}).get("hardware_passthrough")),
        },
        # 源表指纹用于「历史报价可精确重算」。源表本身是**可选**的
        # （业务侧估算表，仅供对照），缺失时记明缘由而不是留一个空字段 ——
        # 空字段读起来像「忘了记」，写明缘由才知道是本来就没有。
        "inputs": {
            "software": ({"path": (deal.get("sources") or {}).get("software"),
                          "sha256_16": sha(src_software)}
                         if src_software else
                         {"path": None, "sha256_16": None,
                          "note": "未声明业务侧估算表；工作量法人月取自 BOM 的 effort_basis"}),
            "hardware": ({"path": (deal.get("sources") or {}).get("hardware"),
                          "sha256_16": sha(src_hardware),
                          "price_column": (deal.get("sources") or {}).get("hardware_price_column")}
                         if src_hardware else
                         {"path": None, "sha256_16": None,
                          "note": "硬件取自 device-config，无源工作簿"}),
        },
        "totals": {
...
def main() -> None:
    ...
    # **源表对照是可选的**。`sources.software` 指向业务侧的估算表，用于
    # 「引擎算的数 vs 业务侧估的数」逐行对照 —— 那是柳州项目的做法，
    # 连 sheet 名（平台开发/底座开发）都是它特有的。第二个行业没有这份表，
    # 也不该被迫造一份：缺它时 sw 从 BOM 的 effort_basis 重建
    # （下方 3563 行附近本就有这条路径），只是少了对照列。
    _src = (deal.get("sources") or {}).get("software")
    if _src:
        sw = read_software(root / _src, pack, source_alias(root / "bom"))
    else:
        sw = {"systems": {}, "issues": [], "no_source_table": True}
        print("  ⚠ deal.sources.software 未声明 —— 工作量法各段的人月"
              "从 BOM 的 effort_basis 取，无业务侧估算表可对照")
    # 硬件的源**显式声明**，不靠「文件在不在」来切。
    # 靠文件存在性切换的话，配置文件被改名/挪走会静默回落到源工作簿 ——
    # 两条链路的数不一样，而且不会有任何地方报错。
    hw_src = (deal.get("sources") or {}).get("hardware_source", "workbook")
    ...
    _write_lock(a.deal.parent / "deal.lock.json", deal, pack, sw=sw, hw=hw,
                ...
                fp_cross=fp_cross,
                src_software=(root / _src) if _src else None,
                src_hardware=((root / deal["sources"]["hardware"])
                              if (deal.get("sources") or {}).get("hardware") else None))
