...
COLUMN_ROLES: dict[str, list[str]] = {
    "total": ["成本总价", "对外总价", "成本总价（元）", "对外总价（元）",
              ...
              "增量成本",
              # 送审包（gov_sheet）的金额列。这几个是 `assert_roles_parseable`
              # 在真实生成时报出来的 —— 此前「年度金额（元）」认不出，
              # 运营期那张表的钱根本没进原总额。
              "软件开发费用（元）", "年度金额（元）"],
    "unit_price": ["成本单价", "单价（元）", "单价", "对外单价", "人天单价"],
    "person_days": ["人/天", "人天", "人日", "工时（人天）", "增量工时"],
    #: 表里**已经写明**的功能点数。有它就不该再从人天倒推 ——
    #: 倒推是给「只给人天、不给 FP」的外部报价用的兜底，而我们自己生成的
    #: 送审表直接给 FP 且没有人天列，倒推在那里只会得 0。
    "fp": ["调整后功能点", "计入开发费功能点", "功能点", "功能点数",
           "未调整功能点", "FP"],
    "detail": ["建设详情", "详情", "建设详情（定位）", "建设内容",
               "功能描述", "分项名称", "名称",
               # 送审包（gov_sheet）的描述列。认不出的后果不是报错而是
               # `classify_fp_row` 拿到空串 —— 全表按兜底类别归类，静默失准。
               "明细", "功能点名称", "费用大类"],
    "qty": ["数量", "台数", "套数"],
}
