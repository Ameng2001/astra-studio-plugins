...
class Bom:
    ...
    def load(cls, root: Path) -> Bom:
        root = Path(root)
        # VERSION 向上找：v2 的层内分组（shared/platform-base、shared/llm-infra）
        # 共用层根的 VERSION —— 同一层的内容一起发版，分组各自编版本会让
        # 「这个商机用的是哪一版共享层」失去单一答案。
        vp = root / "VERSION"
        if not vp.exists():
            for up in root.parents:
                if (up / "VERSION").exists():
                    vp = up / "VERSION"
                    break
    ...
    @classmethod
    def compose(cls, bom_root: Path, spec: dict[str, Any]) -> Bom:
        """按 deal.compose 组合 shared 与 vertical 两层，并施加商机级裁剪。

        **裁剪发生在加载期，不改任何产品级文件**（架构规矩二）。v1 曾用
        「在 BOM 里置 status=deprecated」表达商机级范围决策 —— 单行业尚可，
        多行业下会让其他行业的商机跟着丢功能：产品级只回答「这个平台有什么」，
        商机级才回答「这次交付哪些」。
        ...
        spec 形如：
            shared:
              platform-base:
                version: 0.1.0
                include: [AIOT平台, 全域数据平台]
                exclude_modules: {AI计算平台: [费用管理]}
              llm-infra: {version: 0.1.0, include: [...]}
            vertical:
              preschool: {version: 0.1.0, include: [...]}

        `include` 省略 = 全取该层。`version` 与实际 VERSION 不符时硬失败 ——
        版本对不上就重算，是「任何历史报价可精确重算」的前提。
        """
        bom_root = Path(bom_root)
        parts: list[tuple[str, Path]] = []
        for grp, sub in (spec.get("shared") or {}).items():
            parts.append((f"shared/{grp}", bom_root / "shared" / grp))
        for vert, sub in (spec.get("vertical") or {}).items():
            for grp in ("apps", "llm"):
                d = bom_root / "verticals" / vert / grp
                if d.exists():
                    parts.append((f"verticals/{vert}/{grp}", d))
        if not parts:
            raise BomError("deal.compose 未声明任何层 —— 组合为空不会报错，"
                           "只会算出一个金额为 0 的套表")

        items: list[BomItem] = []
        tax: dict[str, Any] = {"schema_version": SCHEMA_VERSION, "product_lines": {}}
        vers: dict[str, str] = {}
        for label, d in parts:
            layer_key = label.split("/")[0]
            cfg = ((spec.get("shared") or {}).get(label.split("/", 1)[1])
                   if layer_key == "shared"
                   else (spec.get("vertical") or {}).get(label.split("/")[1])) or {}
            sub = cls.load(d)
            want = cfg.get("version")
            if want and want != sub.version:
                raise BomError(
                    f"{label} 声明 version={want}，实际 {sub.version} —— "
                    f"版本对不上就无法保证历史报价可重算，请更新 deal 或重建基线")
            vers[label] = sub.version
            inc = cfg.get("include")
            exm = cfg.get("exclude_modules") or {}
            for it in sub.items:
                s = it.path.system
                if inc is not None and s not in inc:
                    continue
                if s in exm and (it.path.l1 or "") in (exm[s] or []):
                    continue
                items.append(it)
            for pl, pd in (sub.taxonomy.get("product_lines") or {}).items():
                keep = {k: v for k, v in (pd.get("systems") or {}).items()
                        if inc is None or k in inc}
                if keep:
                    tgt = tax["product_lines"].setdefault(
                        pl, {k: v for k, v in pd.items() if k != "systems"} | {"systems": {}})
                    tgt["systems"].update(keep)

        # **同层 ID 唯一是硬约束**（架构规矩一）。v1 曾有 110 个重复 ID 导致
        # 472 UFP 重复计数与共创记录错配；跨行业后规模会放大到数千条。
        seen: dict[str, str] = {}
        dup: list[str] = []
        for it in items:
            if it.id in seen:
                dup.append(f"{it.id}（{seen[it.id]} ⟷ {it.path.system}）")
            seen[it.id] = it.path.system
        if dup:
            raise BomError(
                f"组合后出现 {len(dup)} 个重复条目 ID，例如 {dup[:5]}。\n"
                f"  同层 ID 必须唯一 —— 重复会导致 UFP 重复计数，且飞书共创记录"
                f"按 ID 对齐时会命中错误的一侧。")

        # **版本串要能当目录名用**：基线目录是 `<pack>@bom-<ver>`，
        # 组合串里的 `/` 会被当成路径分隔、`+` 也不宜出现在目录名里。
        # 这里出简短摘要（各层版本去重后拼接），完整组合记在 compose_versions，
        # 由 deal.lock.json 保存 —— 摘要用于寻址，完整组合用于重算。
        uniq = sorted(set(vers.values()))
        ver = uniq[0] if len(uniq) == 1 else "-".join(uniq)
        b = cls(ver, items, tax)
        b.root = bom_root
        b.compose_versions = vers
        return b
